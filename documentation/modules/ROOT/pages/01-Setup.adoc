...
IMPORTANT: Change `/mnt` for the directory where you have enough space (>370GB)
...
- Install some binaries (oc, kubectl, opm, wget, podman ):
...
----
This command will route the traffic from our laptop/PC to the network `192.168.122.0/24` (libvirtd network in our hypervisor) via ssh connection.
...
Follow next steps to create all registries:
